...
<html lang="en">
   <head>
      ...
      <%@page import="com.ibm.twa.bluemix.samples.managers.CloudantManager"%>

		<%!
			//Session key to register in the session
			private static final String WORKLOAD_APP_SESSION_KEY = "SIMPLE_WORKLOAD_APP";
		%>
		<%
			CloudantManager cm;
			if (session.getAttribute(WORKLOAD_APP_SESSION_KEY)!=null){
				cm = (CloudantManager) session.getAttribute(WORKLOAD_APP_SESSION_KEY);
			}else{
				cm = new CloudantManager();
				session.setAttribute(WORKLOAD_APP_SESSION_KEY,cm);
			}
		%>
   </head>
    <body>
         ...
         Submitting request...
      	...
      	if(request.getParameter("formSubmitted") != null){
			...
			if (cm.isConnected()) { 
				...
				cm.postSubmission(address, subject, body);
				if(cm.isSubmitted()){
				%>
				<script>
				...
				%>
				<script>
				 	alert("Error during the submission (Cloudant not connected)");
				</script>
				<%
			}
      	}
